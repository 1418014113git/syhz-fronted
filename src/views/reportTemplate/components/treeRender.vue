<template>
  <span class='custom-tree-node'>
    ...
    <span class='treeBtn' v-if='ISSHOWBTN'>
      <!--带有按钮权限控制方法的-->
      <el-button  title="添加" class='el-icon-plus' v-if="controlBtnAdd(STORE,DATA,NODE)"  type='primary' circle  @click.stop="nodeAdd(STORE,DATA,NODE)"></el-button>
      <el-button  title="编辑" v-if="DATA.deleteable!=='1' && controlBtnEdit(STORE,DATA,NODE)"  class='el-icon-edit-outline' type='primary' circle  @click.stop="nodeEdit(STORE,DATA,NODE)"></el-button>
      <el-button  title="删除" v-if="DATA.deleteable!=='1' && controlBtnDelete(STORE,DATA,NODE)"  class='el-icon-delete' type='primary' circle  @click.stop="nodeDel(STORE,DATA,NODE)"></el-button>
    </span>
  </span>
</template>
