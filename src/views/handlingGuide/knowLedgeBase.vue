    ...
    <el-row type="flex" justify="center">
      <el-col :span="24">
        <el-row>
          <el-col :span="4">
            <el-card>
              <div slot="header"><span>知识分类</span></div>
              <!--<div class="all">全部（{{ totalData.total }}）</div>-->
              <ul><li class="menu" @click="handleMenuClick('1')" :class="active === '1' ? 'activeSpan' : ''"><i class="el-icon-picture"></i><span>法律法规（{{ totalData.type1 }}）</span></li></ul>
              <ul><li class="menu" @click="handleMenuClick('2')" :class="active === '2' ? 'activeSpan' : ''"><i class="el-icon-document"></i><span>行业标准（{{ totalData.type2 }}）</span></li></ul>
            ...
            <el-card>
              <el-form :inline="true" :model="filters" label-width="80px">
                ...
                <el-form-item label="类别">
                  <el-select v-model="filters.articleType" placeholder="请选择" clearable style="width: 222px">
              ...
              <el-table :data="curriculumData" v-loading="listLoading" style="width: 100%; margin-top: 5px;"  :max-height="countHeight" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="type" label="知识分类" width="100px">
                  <template slot-scope="scope">
                    <span v-if="scope.row.type === '1' || active === '1'">法律法规</span>
                    <span v-if="scope.row.type === '2' || active === '2'">行业标准</span>
                    <span v-if="scope.row.type === '3' || active === '3'">规范制度</span>
                    <span v-if="scope.row.type === '4' || active === '4'">案例指引</span>
                  </template>
                </el-table-column>
                <el-table-column prop="articleType" label="类别" width="80px">
                  <template slot-scope="scope">
                    <span v-if="scope.row.articleType === 1">食品</span>
                    <span v-if="scope.row.articleType === 2">药品</span>
                    <span v-if="scope.row.articleType === 3">环境</span>
  ...
  export default {
    ...
    methods: {
      ...
      uploadFile() {
        ...
        if (this.active === '2') {
          this.$gotoid('/handlingGuide/hybz/add', JSON.stringify(para))
        }
        if (this.active === '3') {
      ...
      handleRowView(index, row) {
        ...
        if (this.active === '2') {
          this.$gotoid('/handlingGuide/hybz/detail', JSON.stringify(para))
        }
        if (this.active === '3') {
          this.$gotoid('/handlingGuide/gfzd/detail', JSON.stringify(para))
        }
        if (this.active === '4') {
          this.$gotoid('/handlingGuide/alzy/detail', JSON.stringify(para))
        }
      },
      handleRowEdit(index, row) {
        ...
        if (this.active === '1') {
          this.$gotoid('/handlingGuide/flfg/edit', JSON.stringify(para))
        }
        if (this.active === '2') {
          this.$gotoid('/handlingGuide/hybz/edit', JSON.stringify(para))
        }
        if (this.active === '3') {
          this.$gotoid('/handlingGuide/gfzd/edit', JSON.stringify(para))
        }
        if (this.active === '4') {
          this.$gotoid('/handlingGuide/alzy/edit', JSON.stringify(para))
        }
      },
        ...
        this.$refs.auditForm.validate(valid => {
          if (valid) {
            ...
            let para = {
              ...
              belongType: this.auditForm.articleType,
              belongSys: '1',
...
<style>
  ...
  .knowLedgeBase .menu.activeSpan, .knowLedgeBase .menu.activeSpan:hover {
  ...
  .knowLedgeBase .el-card.is-always-shadow .el-card__body > div.all{
    height: 50px;
  }
  .knowLedgeBase .el-card.is-always-shadow .el-card__body ul{
    margin-left: 20px;
  }
  .knowLedgeBase .package_customClass {
    width: 25%;
  }
  .knowLedgeBase .package_customClass .el-message-box__input {
    padding-left: 36px;
    padding-right: 12px;
  }
  .knowLedgeBase .package_customClass .el-message-box__input .el-textarea {
    width: 100%;
  }
  .knowLedgeBase .auditClass{
    padding: 2px;
    top: 4px;
    position: absolute;
    margin-left: 14px;
  }
  .knowLedgeBase .auditClass svg{
    width: 18px;
    height: 18px;
  }
  .knowLedgeBase .el-dialog__wrapper.audit_dialog{

  }
  .knowLedgeBase .el-dialog__wrapper.audit_dialog .el-dialog .el-dialog__body{
    padding-left: 40px;
    padding-right: 40px;
  }
  .knowLedgeBase .url_text{
    cursor: pointer;
  }
</style>
