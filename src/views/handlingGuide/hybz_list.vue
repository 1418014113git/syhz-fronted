        ...
        <el-col :span="24" style="padding-top: 10px; padding-bottom: 0;">
          <el-form-item style="width: 43%; margin-left: 1px; padding-left: 9px;">
            <el-input size="large" placeholder="在法规中搜索" v-model="filters.search">
              <el-button slot="append" v-if="$isViewBtn('129101')" v-on:click="query(true,true)" icon="el-icon-search" style="width: 100px; font-size: 20px; color: #fff; line-height: 30px;background-color: #1e98d2;"></el-button>
            </el-input>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" v-if="$isViewBtn('129102')" @click="add" style="padding: 12px 35px;">添加</el-button>
          </el-form-item>
        </el-col>
  ...
  export default {
    ...
    methods: {
      ...
      query(flag, hand) {
        ...
        this.$update('industryInfo/query', para).then((response) => {
          this.loading = false
      ...
      detail(id) {
        if (this.$isViewBtn('129103')) {
          this.$gotoid('/handlingGuide/hybz/detail', JSON.stringify({ id: id }))
        }
      },
      add() {
        const para = {
...
<style>
  ...
  .hybzList .lineStyle:last-child{
    border-bottom: 0;
  }
  .hybzList .lineStyle .el-col {
    padding: 5px;
  }
  .hybzList .el-input-group__append {
    background: #0082e6;
    border: none;
  }
  .hybzList .el-form-item__content {
    width: 100%;
  }
  .hybzList .tabsDiv {
    width: 50%;
