...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      callback() {
        if (this.callBack === '') {
          this.$gotoid('/handlingGuide/flfgList', JSON.stringify(this.filters))
        } else {
        ...
        this.$refs.form.validate(valid => {
          if (valid) {
            ...
            if (this.id !== '') {
              ...
              para.subType = draft
              para.id = this.id
              this.$update('lawInfo/update', para).then(response => {
                this.loading = false
                this.$message({
                  message: '修改成功',
                  type: 'success'
              ...
              this.$update('lawInfo/save', para).then(response => {
                this.loading = false
                this.$message({
                  message: '添加成功',
                  type: 'success'
      ...
      fileBeforeUpload(file) {
        ...
        // }
        // const response = await this.$queryAsyns('knowledgeenclosurebyname', { belongMode: '1', enName: file.name.substring(0, file.name.lastIndexOf('.')), enType: this.uploadFileType })
      ...
      uploadCheck(file, fileList) {
        ...
        //   const checkFlag = true
        //   if (!this.checkEnName(file, checkFlag)) {
      ...
      async checkEnName(file, flag) {
        // 校验文件名称是否重复
        const response = await this.$queryAsyns('knowledgeenclosurebyname', { belongMode: '1', enName: file.name.substring(0, file.name.lastIndexOf('.')), enClass: file.name.substring(file.name.lastIndexOf('.'), file.name.length) })
        if (response.data.data !== null && response.data.data.length > 0) {
          this.$alert('您上传的资料在平台上已经存在，需要确认平台上已有的资料是否和您要上传的相同，如果不同，请修改资料名称后重新上传！', '提示', {
            confirmButtonText: '知道了',
            callback: action => {
...
.flfgEdit .el-form-item {
	margin-bottom: 16px;
}
.flfgEdit .el-select,
.flfgEdit .el-date-editor--date {
  width: 100%;
}
.flfgEdit .el-upload.el-upload--text, .flfgEdit .el-upload-dragger{
  width: 100%;
}
  .flfgEdit .caseEdit .ql-image{
    display: none;
  }
</style>
