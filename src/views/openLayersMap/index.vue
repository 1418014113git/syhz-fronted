<template>
   <!-- 高发行业分析地图 -->
  <div>
     <img src="@/assets/icon/back.png"  class="goBack" @click="toback" v-if="AJBH || ajid">   <!--返回-->
...
export default {
  ...
  methods: {
    ...
    initMap() {
      ...
      _that.map.on('pointermove', function(e) {
        ...
        _that.map.forEachFeatureAtPixel(pixel, function(feature) {
          ...
          content.innerHTML = '<p class="pointText"><span>案件名称：</span>' + feature.values_.ajmcName + '</p><p class="pointText"><span>案件编号：</span>' + feature.values_.ajbh + '</p>'
    ...
    handleAddBatchFeature() {
      ...
      for (let i = 0; i < this.coordinates.length; i++) {
        ...
        const feature = new Feature({
          ...
          ajmcName: _that.coordinates[i].AJMC,
          ajbh: _that.coordinates[i].AJBH
        })
        // 设置ID
        feature.setId(i)
        // 设置样式
    ...
    getCoordinates() {
      this.loading = true
      this.$query('gfdqajfxzb', {}).then((response) => {
        this.loading = false
        if (response.code === '000000') {
          this.coordinates = response.data
          for (var i = 0; i < this.coordinates.length; i++) {
            var pointer = this.coordinates[i].x.substring(0, 3)
