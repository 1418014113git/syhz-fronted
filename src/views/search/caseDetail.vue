...
<script>
  import LeftNav from '../caseFile/components/leftNav' // 左侧菜单
  import AjTitle from '../caseFile/components/ajTitle' // 右侧--案件信息
  import AjInfo from '../caseFile/components/ajInfo' // 右侧--案件信息
  import AjStatus from '../caseFile/components/ajStatus' // 右侧--案件进度
  import AjSynopsis from '../caseFile/components/ajSynopsis' // 右侧--案件梗概
  import guideDetect from '../caseFile/components/guideDetect' // 右侧--引导侦查
  import FaAddress from '../caseFile/components/faAddress' // 右侧--发案地址
  import RelationMesh from '../caseFile/components/relationMesh' // 右侧--关系挖掘
  import InfoSupplement from '../caseFile/components/infoSupplement' // 信息补采
  import BzPush from '../caseFile/components/bzPush' // 右侧--比中推送
  import ReceivePolice from '../caseFile/components/receivePolice' // 接处警
  // import CluesAssociate from './components/cluesAssociate' // 线索关联
  import SpyonCooperation from '../caseFile/components/spyonCooperation' // 侦查协作
  import AjParallel from '../caseFile/components/ajParallel' // 案件串并
    ...
    methods: {
      ...
      init() { // 根据参数类型，请求不同的接口
        if (JSON.parse(sessionStorage.getItem(this.$route.path)).id) {
          this.AJID = JSON.parse(sessionStorage.getItem(this.$route.path)).id
          if (this.$route.query.isRl) {
            this.isRl = '1'
          }
          this.detail(JSON.parse(sessionStorage.getItem(this.$route.path)).id)
        }
      },
      detail(id) {
        this.loading = true
