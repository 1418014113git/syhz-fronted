...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      detail() {
        this.loading = true
        this.$query('clue/getOne/', { id: JSON.parse(sessionStorage.getItem(this.$route.path)).id }).then(response => {
      ...
      getShare() {
        this.loading = true
        this.$query('clue/getShareDeptByClueId/', { clueId: JSON.parse(sessionStorage.getItem(this.$route.path)).id }).then(response => {
