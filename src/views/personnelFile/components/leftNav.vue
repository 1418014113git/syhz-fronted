<template>
  ...
  <div class="leftNavPer" v-loading="loading">
    <div class="NameWrap">
      <div class="NameBox">
        <p class="p1" v-if="baseInfo.xm">{{baseInfo.xm}}</p>
        <!-- <p class="p2">
          <img src="/static/image/personFile_images/guanzhu.png">
          <span>关注</span>
        </p> -->
      </div>
    </div>
    <div class="NamePhoto">
      <div class="photoWrap">
        <div class="photoBox">
          <img v-if="baseInfo.xp" :src="'data:image/jpeg;base64,'+baseInfo.xp">
          <img v-else src="/static/image/personFile_images/defaultUuser.png">
        </div>
        <div class="personType1" v-if="isKeyPerson">重点人员</div>
    ...
    <ul class="navList">
      <li v-for="(item,index) in navList" :key="index" :class="{'on':curIndex === index}" @click="clickNav(item.class,index)" >
        <div>
          <img :src="item.icon">
          <span class="navName" :class="{'gray':item.number === 0 && index!==0 && index!==1 && index!==5}">{{item.name}}</span>
          <span class="number"  v-if="item.number>0 && item.number<99 && index!==0 && index!==1 && index!==5" >{{item.number}}</span>
          <span class="number pdm"  v-else-if="item.number>99 && index!==0 && index!==1 && index!==5">99+</span>
...
export default {
  ...
  data() {
    return {
      ...
      curIndex: 0, // 菜单当前索引
      isKeyPerson: false, // 是否是重点人员
      loading: false, // 页面加载进度条
      navList: [
        { icon: '/static/image/personFile_images/jbxx.png', name: '基本信息', number: 0, type: 't0', class: 'jbxx' },
        { icon: '/static/image/personFile_images/zdlx.png', name: '重点类别', number: 0, type: 't1', class: 'zdlx' },
        { icon: '/static/image/personFile_images/saxx.png', name: '涉案信息', number: 0, type: 't2', class: 'saxx' },
        { icon: '/static/image/personFile_images/xzcf.png', name: '行政处罚', number: 0, type: 't3', class: 'xzcf' },
        { icon: '/static/image/personFile_images/zjgj.png', name: '最近轨迹', number: 0, type: 't4', class: 'zxgj' },
        { icon: '/static/image/personFile_images/gxwj.png', name: '关系挖掘', number: 0, type: 't5', class: 'gxwj' },
        { icon: '/static/image/personFile_images/clxx.png', name: '车辆信息', number: 0, type: 't6', class: 'clxx' },
        { icon: '/static/image/personFile_images/fwxx.png', name: '服务信息', number: 0, type: 't7', class: 'fwxx' },
        { icon: '/static/image/personFile_images/sbrs.png', name: '社保人事', number: 0, type: 't8', class: 'sbrs' }
      ],
      cardNumber: ''
  ...
  methods: {
    ...
    getzdryhc() {
      ...
      //     if (arryLength > 0) {
      //       this.isKeyPerson = true
      //     }
      //     this.navList.forEach(item => {
      //       if (item.type === 't1') {
      //         item.number = arryLength
      //       }
      //     })
      //   }
      // }).catch(() => {
      ...
      var arry = infoData.LBBJ ? infoData.LBBJ.split(',') : []
      var arryLength = arry.length
      if (arryLength > 0) {
        this.isKeyPerson = true
      }
    ...
    clearSession() {
      ...
      sessionStorage.removeItem('t6')
      sessionStorage.removeItem('t7')
      sessionStorage.removeItem('t8')
    }
  },
  mounted() {
