    ...
    <div class="verifyInfo pubStyle">
      ...
      <el-table :data="listData" style="width: 100%;" v-loading="listLoading" class="statisticCollect" max-height="156">
        ...
        <el-table-column label="操作" align="center" width="100">
          <template slot-scope="scope">
            <el-button size="mini" title="重新申请"  type="primary" circle  v-if="(((curDept.depType === '4' && curDept.parentDepId===scope.row.createDept) || (curDept.depType !== '4' && curDept.id===scope.row.createDept)) && scope.row.wflowStatus===4)" @click="handleApply(scope.$index, scope.row)"><svg-icon icon-class="shenqing"></svg-icon></el-button>
            <el-button size="mini" title="审核"  type="primary" circle  v-if="(Number(curDept.depType) < 3 && curDept.id===scope.row.acceptDeptId && scope.row.wflowStatus===1)" @click="handleVerify(scope.$index, scope.row)"><svg-icon icon-class="audit"></svg-icon></el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-row>
      ...
      <el-dialog title="审核" :visible.sync="isShowshDialog"  class="stshForm" :close-on-click-modal="false">
        <audit-com  :isShowDialog="isShowshDialog"  @closeDialog="closeshDialog" :row="curRow" :info="baseInfo"></audit-com>
      </el-dialog>
    </div>
  </section>
</template>
...
export default {
  ...
  methods: {
    ...
    controlBtn(data) { // 遍历列表信息，控制详情页上方的申请，审核按钮
      ...
      if (data.length > 0) {
        data.forEach(item => {
          if ((this.curDept.depType === '4' && this.curDept.parentDepId === item.createDept) || (this.curDept.depType !== '4' && this.curDept.id === item.createDept)) { // 当前登录的是派出所时，用他的父级单位的id去判断
            Bus.$emit('isShowsqbtn', true) // 控制详情页上方的申请按钮显隐
          }
          if (Number(this.curDept.depType) < 3 && this.curDept.id === item.acceptDeptId && item.wflowStatus === 1) {
