<template>
<!-- 关系挖掘 -->
  <div class="relationMesh">
    <title-pub :title="title" :url="url" :jumpType="1" :param="moreParam"></title-pub>
    <div class="mesh" v-loading="meshloading">
...
export default {
  ...
  methods: {
    ...
    clickDetail: function() {
      ...
      if (params) {
        ...
        if (data.type === '1') {
          ...
          _this.$router.push({
            // path: '/inforCollection/personnelDetailAll/',
          ...
          _this.nodeMenu = false
          _this.$router.push({
            // path: '/caseManage/detailetl/' + data.id
    ...
    queryService: function(url, parentData, parentIndex) {
      ...
      _this.meshloading = true
      axios.post(url).then(response => {
        if (response.status === 200) {
          _this.meshloading = false
          var data = response.data
          if (data && data.code === '000000') {
    ...
    setParam() {
      if (this.AJID) {
        if (this.isRls) {
          this.moreParam = JSON.stringify({ type: '2', bm: this.AJBH, id: this.AJID, source: this.source, isRl: this.isRls }) // 跳转到关系挖掘主页传递的参数
        } else {
          this.moreParam = JSON.stringify({ type: '2', bm: this.AJBH, id: this.AJID, source: this.source }) // 传递案件id
        }
      } else if (this.AJBH) {
        if (this.interFaceType) {
          this.moreParam = JSON.stringify({ type: '2', bm: this.AJBH, source: this.source, interfaceType: this.interFaceType }) // 传递案件编号
        } else if (this.isRls) {
          this.moreParam = JSON.stringify({ type: '2', bm: this.AJBH, source: this.source, isRl: this.isRls }) // 传递案件编号
        }
      } else {
        this.moreParam = JSON.stringify({ type: '2', bm: this.AJBH })
      }
    }
    // getRelation() { // 获取关系挖掘数据
    //   var _this = this
    //   _this.AJID = _this.ajid
    //   _this.interFaceType = _this.type
    //   _this.isRls = _this.Rl
    //   _this.meshloading = true // 页面加载loading
    //   if (_this.AJBH && _this.queryType) { // 正式数据
    //     if (_this.queryType === 2) {
    //     // 从详情页面返回的是案件编号
    //       _this.searchDataByAjbh(_this.AJBH)
    //     } else if (_this.queryType === 3) {
    //       // 从详情页面返回的是线索编号
    //       _this.searchDataByQBXSBH(_this.AJBH)
    //     }
    //     _this.setParam()
    //   }
    //   // // _this.searchDataByIdCardNo(_this.AJBH) // 测试数据
    // }
  },
  mounted() {
    this.init()
