      ...
      <el-col :span="16" style="padding: 5px 20px;">
        <div v-loading="listLoading">
          ...
          <div class="dept_ineg_num">
            ...
            <el-button type="primary" size="small" @click="excelImport" v-if="$isViewBtn('106003') && enableBtn && ifSign && inegState"
                       style="float: right;">批量导入
            </el-button>
            <el-button type="primary" size="small" v-on:click="handleAdd" v-if="$isViewBtn('106005') && enableBtn && ifSign && inegState"
                       style="float: right; margin-right: 10px;">添加情报
            </el-button>
          </div>
...
<script>
  ...
  export default {
    name: 'qbxsMapping',
    props: ['deptIds', 'depts', 'state', 'ifSign', 'speTaskId'],
    data() {
      return {
        enableBtn: false,
        inegState: false,
    ...
    methods: {
      ...
      qbMappSave() {
        ...
        this.$save('qbxsmapping', para).then((response) => {
...
<style scoped>
  ...
  .qbxsMapping .dept_name {
    font-weight: bold;
    font-size: 22px;
  }

  .qbxsMapping .dept_ineg_num span {
    padding: 5px 35px 5px 0;
  }

  .qbxsMapping .filter-tree {
    width: 550px;
  }
</style>
